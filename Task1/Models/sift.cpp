...
void SIFTX:: char_to_float(cv::Mat& Image1, cv::Mat&Image2) {
...
cv::Mat SIFTX:: subsample(cv::Mat& I, float factor) {
    cv::Mat tmp = I, dst = I;
...
vector<cv::Mat>SIFTX:: get_pyramid(cv::Mat& I, int levels) {
    cv::Mat tmp = I, dst = I;
...
void SIFTX:: get_scale_space(cv::Mat& I, float sigma,vector<vector<cv::Mat>>& scale_space,float k) {
    vector<cv::Mat> pyramid = SIFTX::get_pyramid(I, 4);
    for (int i = 0;i < 4;i++) {
...
void SIFTX:: get_DoG(vector<vector<cv::Mat>>& scale_space, vector<vector<cv::Mat>>& DoG) {
...
bool SIFTX::is_max(cv::Mat& I1, cv::Mat& I2, cv::Mat& I3, int row, int col) {
    for (int i = -1;i <= 1;i++) {
        for (int j = -1;j <= 1;j++) {
            if (i == 0 && j == 0) continue;
...
bool SIFTX:: is_min(cv::Mat& I1, cv::Mat& I2, cv::Mat& I3, int row, int col) {
    for (int i = -1;i <= 1;i++) {
        for (int j = -1;j <= 1;j++) {
            if (i == 0 && j == 0) continue;
...
void SIFTX:: get_keypoints(vector<vector<cv::Mat>>& DoG, vector<vector<cv::Mat>>&keypoints) {
...
void SIFTX:: remove_low_contrast(vector<vector<cv::Mat>>& keypoints, vector<vector<cv::Mat>>& DoG, float contrast_threshold) {
    for (int i = 0;i < 4;i++) {
...
void SIFTX:: remove_edges(vector<vector<cv::Mat>>& keypoints, vector<vector<cv::Mat>>& DoG, float edge_threshold) {
    for (int i = 0;i < 4;i++) {
        cv::Mat second = DoG[i][1];
        cv::Mat third = DoG[i][2];
...
void SIFTX:: draw_keypoints(cv::Mat& I, vector<vector<cv::Mat>>& keypoints) {
    for (int k = 0;k < 4;k++) {
